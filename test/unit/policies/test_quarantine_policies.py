...
from torch_june.policies import QuarantinePolicy, QuarantinePolicies, quarantine_policies
from torch_june.timer import Timer
from torch_june.message_passing import InfectionPassing


class TestQuarantinePolicies:
    def test__match_symptoms(self):
        ...
        quarantine_array = quarantine.apply(
            timer=timer, symptom_stages=symptom_stages
        )
    ...
    def test__integration(self, inf_data):
        ...
        mp = InfectionPassing(log_beta_company = torch.tensor(3.0), log_beta_household=torch.tensor(3.0))
        quarantine = QuarantinePolicy(
            stage_threshold=3, start_date="2022-02-01", end_date="2022-03-15"
        )
        ret = mp(data=inf_data, timer=timer, quarantine_policies = QuarantinePolicies([quarantine]))
        assert np.isclose(ret.sum().detach(), n_agents) # No-one gets infected since they all quarantine
        while not timer.is_weekend:
            next(timer)
        ret = mp(data=inf_data, timer=timer, quarantine_policies = QuarantinePolicies([quarantine]))
        assert np.isclose(ret.sum().detach().item(), 10.0) # people living in the same household get infected, seed survives
