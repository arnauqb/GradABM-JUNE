    ...
    def test__beta_reduction(self, sd, timer):
        while timer.date < timer.final_date:
            if timer.date < datetime.datetime(2022, 2, 5):
                assert sd.apply(beta=3, name="school", timer=timer) == 3 * 0.3
                assert sd.apply(beta=2, name="company", timer=timer) == 1
            else:
                assert sd.apply(beta=3, name="school", timer=timer) == 3
                assert sd.apply(beta=2, name="company", timer=timer) == 2
            assert sd.apply(beta=3, name="leisure", timer=timer) == 3
            next(timer)
